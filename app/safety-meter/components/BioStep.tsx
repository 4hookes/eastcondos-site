...
export function BioStep({ buyer1, buyer2, cashSavings, onBuyer1Change, onBuyer2Change, onCashSavingsChange, onNext }: Props) {
  ...
  function BuyerFields({ buyer, onChange, label, kicker }: { buyer: BuyerBio; onChange: (b: BuyerBio) => void; label: string; kicker: string }) {
    return (
      <div className="border border-charcoal bg-cream p-5 sm:p-6">
        ...
        <div className="font-serif text-[20px] text-charcoal mb-4" style={{ letterSpacing: "-0.02em" }}>{label}</div>
          ...
          <div>
            ...
            <div className={`${styles.numberInput} ${styles.noPrefix}`}>
              <input
                ...
                min={21}
                max={70}
                onChange={(e) => onChange({ ...buyer, age: num(e.target.value) })}
              />
            </div>
          </div>
          <div>
            ...
            <div className={styles.numberInput} data-prefix="S$">
              <input
                ...
                min={0}
                step={500}
                onChange={(e) => onChange({ ...buyer, fixedSalary: num(e.target.value) })}
              />
            </div>
          </div>
          <div>
            ...
            <div className={styles.numberInput} data-prefix="S$">
              <input
                ...
                min={0}
                step={1000}
                onChange={(e) => onChange({ ...buyer, variableAnnual: num(e.target.value) })}
              />
            </div>
          </div>
          <div>
            ...
            <div className={styles.numberInput} data-prefix="S$">
              <input
                ...
                min={0}
                step={100}
                onChange={(e) => onChange({ ...buyer, obligations: num(e.target.value) })}
              />
            </div>
            <span className="block text-[11px] text-gray-500 mt-1 font-serif italic">Car loans, credit card minimums, etc.</span>
          </div>
          <div>
            ...
            <div className={styles.numberInput} data-prefix="S$">
              <input
                ...
                min={0}
                step={1000}
                onChange={(e) => onChange({ ...buyer, oaBalance: num(e.target.value) })}
              />
            </div>
          </div>
        </div>
      ...
      <div className="flex items-center justify-between mb-6 flex-wrap gap-3">
        <div>
          <div className="flex items-center gap-3 mb-1">
            ...
            <span className="text-[10px] uppercase tracking-[0.26em] text-amber-deep">Step 01 · Who's buying</span>
          </div>
        ...
        <div className="flex items-center gap-3">
          ...
          <button
            ...
            onClick={() => {
              ...
              else onBuyer2Change({ name: "", age: 35, fixedSalary: 0, variableAnnual: 0, obligations: 0, cpfYes: true, oaBalance: 0 });
            }}
          />
        </div>
      </div>
      ...
      <div className="mt-6 pt-5 border-t border-dotted border-[#c9bfa3]">
        ...
        <div className={`${styles.numberInput} max-w-xs`} data-prefix="S$">
          <input
            ...
            min={0}
            step={5000}
            onChange={(e) => onCashSavingsChange(num(e.target.value))}
          />
        </div>
        <span className="block text-[11px] text-gray-500 mt-1 font-serif italic">Liquid cash you'll add toward the purchase.</span>
      </div>
